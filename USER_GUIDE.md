...
### Ingestion Commands (0.9.0)

The human half of the learning loop: bring documents, links, and resources in
without routing them through an agent session.

```bash
# A file inside the vault
vault-memory ingest 05\ Dev\ Projects/vault-memory/design.md --vault ~/vault

# A file anywhere else — copied into inbox/ so the daemon stays vault-confined
vault-memory ingest ~/Downloads/paper.pdf --vault ~/vault

# A URL (readability-extracted to markdown)
vault-memory ingest https://example.com/post --vault ~/vault

# Pasted text
vault-memory ingest --text "$(cat notes.txt)" --vault ~/vault

# Everything dropped in inbox/
vault-memory ingest --inbox --vault ~/vault --remove

# What has been ingested (the delta index)
vault-memory ingest --status --vault ~/vault
```

Each source is archived immutably under `raw/{date}-{slug}.md` with provenance
frontmatter, then compiled into `Knowledge/` pages whose claims are tagged
`extracted` / `inferred` / `ambiguous`. A source that collides with a high-trust
page is reported as a conflict and never overwritten.

PDF ingestion needs the optional extra: `pip install 'vault-memory[ingest]'`.

### Lesson Commands (0.9.0)

```bash
# Mine closed sessions into drafts (also runs from the heartbeat when SESSION_MINING=on)
vault-memory sessions mine --limit 5

# The review queue
vault-memory lessons review --project vault-memory

# Approve one (moves it to lessons/, maturity seed -> sapling)
vault-memory lessons promote 2026-09-12-pin-the-provider-version

# Reject one — the reason is fed into the next mining prompt for the project
vault-memory lessons reject 2026-09-12-too-vague \
    --reason "too vague: no evidence from the session"

# What the next agent will be told, ranked
vault-memory lessons list --project vault-memory
```

### Digest & Skills Commands (0.9.0)

```bash
vault-memory digest daily              # 24h overview -> digests/YYYY-MM-DD.md
vault-memory digest weekly             # deep dive    -> digests/YYYY-Www.md
vault-memory digest monthly            # consolidation + skill proposals
vault-memory digest daily --no-summary # skip the LLM summary

vault-memory skills export             # skills/<theme>/SKILL.md
vault-memory skills export --min-corroboration 2   # only corroborated lessons
vault-memory skills list
```

Exported `SKILL.md` bundles are agentskills.io-compatible (`name`,
`description`) and can be read by file-aware agents with or without the daemon
running. A `SKILL.md` that was not generated by this tool is **never**
overwritten — that theme's export is staged under `_working/skills/` instead.

The digests and the miner are both gated (`DIGESTS=on`, `SESSION_MINING=on`) and
run from the heartbeat at 00:00 local time when enabled.
