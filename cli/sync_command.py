...
async def run_full_sync(
    ...
    stats = {"files_done": 0, "files_skipped": 0, "files_errored": 0,
             "chunks_total": 0, "elapsed": 0.0, "rate": 0.0, "current_file": "", "errors": []}
    ...
    with Live(console=console, refresh_per_second=10) as live:
        def _refresh():
            ...
            live.update(Panel(layout, title="[bold cyan]vault-memory sync --full[/]", border_style="cyan"))
...
def _print_summary(summary: dict):
    ...
    table.add_row("Errored:",       f"[red]{summary['files_errored']}[/]" if summary["files_errored"] else "[dim]0[/]")
