...
from .health import router as health_router, mark_ready, mark_degraded, mark_indexing
...
async def lifespan(app: FastAPI):
    ...
    searcher = UnifiedSearch(weaviate=weaviate_client, postgres=pg_client, embedder=embedder)
    ...
    if weaviate_client: weaviate_client.close()
    if pg_client:       pg_client.close()
...
async def search(req: SearchRequest, request: Request):
    results = await request.app.state.searcher.search(
        ...
        include_graph=req.include_graph, include_temporal=req.include_temporal,
...
async def graph_query(entity: str, relationship: Optional[str] = None):
    ...
    cursor.close()
    return {"paths": [{"target": r["target_name"], "relationship": r["relationship_type"]} for r in rows]}


@app.get("/temporal")
async def temporal_query(entity: str, start: str = "2025-01-01", end: str = "2026-12-31"):
    results = await searcher._temporal_search(entity, {"start": start, "end": end})
    return {"results": [r.to_clip() for r in results]}


def start():
    uvicorn.run("daemon.main:app", host="127.0.0.1", port=settings.port, log_level="info")
