...
from daemon.helpers.security import sanitize_for_context
...
async def cognify(
    ...
    safe_text = sanitize_for_context(req.text)

    ollama_cb = get_circuit_breaker("ollama")
    try:
        async def _do_extract():
            return await _extract_triples_with_ollama(
                safe_text, req.entity_types, deps.settings.ollama_url, deps.settings.ollama_model,
            )
