...
class SyncEngine:
    ...
    async def _batch_upsert_relationships(self, relations: List[Tuple[str, str]], edge_source: str = 'canvas'):
            ...
            def _do_batch_insert():
                with self.pg.cursor() as cur:
                    execute_values(
                        ...
                        ''',
                        relations,
                        template=f"(%s, %s, 'connected', '{edge_source}', NOW())"
                    )
            await asyncio.to_thread(_do_batch_insert)
        except Exception as e:
    ...
    async def _sync_canvas_graph(self, rel_path: str, abs_path: Path):
        ...
        # Bolt: Batch store canvas_entities and relationships
        tasks = []
        if result.entities:
            tasks.append(self._batch_upsert_canvas_entities(result.entities))
        if result.edges:
            tasks.append(self._batch_upsert_canvas_relationships(result.edges))
    ...
    async def _batch_upsert_canvas_entities(self, entities: List[CanvasEntity]):
        ...
        if not entities or not hasattr(self.pg, 'cursor') or not callable(self.pg.cursor):
            return
        if execute_values is None:
            logger.warning('batch_upsert_canvas_entities skipped: execute_values unavailable')
            return
        try:
            rows = [(e.canvas_path, e.node_id, e.entity_name, e.entity_type, e.node_text) for e in entities]
            def _do_batch():
                with self.pg.cursor() as cur:
                    execute_values(
                        cur,
    ...
    async def _batch_upsert_canvas_relationships(self, edges: List[CanvasRelationship]):
        """Bolt: Batch insert canvas-sourced relationship records."""
        if not edges or not hasattr(self.pg, 'cursor') or not callable(self.pg.cursor):
            return
        if execute_values is None:
            logger.warning('batch_upsert_canvas_relationships skipped: execute_values unavailable')
            return
        try:
            rows = [(e.source_name, e.target_name, e.relationship_type) for e in edges]
            def _do_batch():
                with self.pg.cursor() as cur:
                    execute_values(
                        cur,
