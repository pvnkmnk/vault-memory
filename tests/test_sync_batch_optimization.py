...
def test_sync_canvas_uses_batch_embedding_and_batch_upsert(tmp_path):
    ...
    rel_canvas_path = str(canvas_path.relative_to(vault_root))
    assert entities_call_args[0].canvas_path == rel_canvas_path
    assert entities_call_args[0].node_id == "n1"
    # The CanvasGraphPipeline might have different logic for entity_name
    # Let's check what it actually returns if it fails, but the error message showed it returned 'First'
